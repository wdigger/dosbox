...
bool CPU_SwitchTask(Bitu new_tss_selector,TSwitchType tstype,Bitu old_eip) {
	...
	if (!new_tss.SetSelector(new_tss_selector)) 
		E_Exit("Illegal TSS for switch, selector=%x, switchtype=%x",(unsigned int)new_tss_selector,tstype);
...
do_interrupt:
					...
					break;		
				default:
					E_Exit("INT:Gate Selector points to illegal descriptor with type %x",(unsigned int)cs_desc.Type());
				}
		...
		case DESC_TASK_GATE:
			...
			return;
		default:
			E_Exit("Illegal descriptor type %X for int %X",(unsigned int)gate.Type(),(unsigned int)num);
		}
	}
	assert(1);
...
void CPU_IRET(bool use32,Bitu oldeip) {
	if (!cpu.pmode) {					/* RealMode IRET */
		...
		case DESC_CODE_R_C_A:	case DESC_CODE_R_C_NA:
			...
			break;
		default:
			E_Exit("IRET:Illegal descriptor type %X",(unsigned int)n_cs_desc.Type());
		}
...
CODE_jmp:
		...
		case DESC_386_TSS_A:
			...
			break;
		default:
			E_Exit("JMP Illegal descriptor type %X",(unsigned int)desc.Type());
		}
	}
	assert(1);
}
...
call_code:
		...
		case DESC_INVALID:			// used by some installers
			...
			return;
		default:
			E_Exit("CALL:Descriptor type %x unsupported",(unsigned int)call.Type());
		}
	}
	assert(1);
}
	...
	if (!cpu.pmode || (reg_flags & FLAG_VM)) {
		...
		if (cpu.cpl==rpl) {	
			...
			case DESC_CODE_R_C_A:case DESC_CODE_R_C_NA:
				...
				break;
			default:
				E_Exit("RET from illegal descriptor type %X",(unsigned int)desc.Type());
			}
RET_same_level:
			if (!desc.saved.seg.p) {
				// borland extender (RTM)
			...
			case DESC_CODE_R_C_A:case DESC_CODE_R_C_NA:
				...
				break;
			default:
				E_Exit("RET from illegal descriptor type %X",(unsigned int)desc.Type());		// or #GP(selector)
			}
...
bool CPU_LTR(Bitu selector) {
	...
	if ((desc.Type()==DESC_286_TSS_A) || (desc.Type()==DESC_386_TSS_A)) {
		...
		if (!cpu_tss.SetSelector(selector)) E_Exit("LTR failed, selector=%X",(unsigned int)selector);
		cpu_tss.desc.SetBusy(true);
		cpu_tss.SaveSelector();
	} else {
