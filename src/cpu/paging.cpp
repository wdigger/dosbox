...
void PAGING_PageFault(PhysPt lin_addr,Bitu page_addr,Bitu faultcode) {
	...
	LOG(LOG_PAGING,LOG_NORMAL)("PageFault at %X type [%x] queue %d",lin_addr,(unsigned int)faultcode,(int)pf_queue.used);
...
//	DEBUG_EnableDebugger();
	...
	DOSBOX_RunMachine();
	pf_queue.used--;
	LOG(LOG_PAGING,LOG_NORMAL)("Left PageFault for %x queue %d",lin_addr,(int)pf_queue.used);
