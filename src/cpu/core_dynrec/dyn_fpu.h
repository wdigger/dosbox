...
static void dyn_fpu_esc1(){
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x00: /* FLD float*/
			...
			break;
		case 0x01: /* UNKNOWN */
			LOG(LOG_FPU,LOG_WARN)("ESC EA 1:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			break;
		case 0x02: /* FST float*/
			dyn_fill_ea(FC_ADDR);
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x05:
			...
			case 0x01:		/* FUCOMPP */
				...
				break;
			default:
				LOG(LOG_FPU,LOG_WARN)("ESC 2:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
				break;
			}
			break;
		default:
...
//				LOG(LOG_FPU,LOG_ERROR)("80267 protected mode (un)set. Nothing done");
				break;
			default:
				E_Exit("ESC 3:ILLEGAL OPCODE group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			}
			break;
		default:
			LOG(LOG_FPU,LOG_WARN)("ESC 3:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			break;
		}
	} else {
		switch(decode.modrm.reg){
		case 0x00:	/* FILD */
			...
			break;
		case 0x01:	/* FISTTP */
			LOG(LOG_FPU,LOG_WARN)("ESC 3 EA:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			break;
		case 0x02:	/* FIST */
			dyn_fill_ea(FC_ADDR); 
...
static void dyn_fpu_esc5(){
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x05:	/*FUCOMP STi */
			...
			break;
		default:
			LOG(LOG_FPU,LOG_WARN)("ESC 5:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			break;
        }
	} else {
		switch(decode.modrm.reg){
		...
		case 0x01:  /* FISTTP longint*/
			LOG(LOG_FPU,LOG_WARN)("ESC 5 EA:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			break;
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x03:  /*FCOMPP*/
			if(decode.modrm.rm != 1) {
				LOG(LOG_FPU,LOG_WARN)("ESC 6:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
				return;
			}
			gen_mov_word_to_reg(FC_OP2,(void*)(&TOP),true);
			gen_add_imm(FC_OP2,1);
...
static void dyn_fpu_esc7(){
	...
	if (decode.modrm.val >= 0xc0) { 
		...
		case 0x04:
			switch(decode.modrm.rm){
				case 0x00:     /* FNSTSW AX*/
					...
					break;
				default:
					LOG(LOG_FPU,LOG_WARN)("ESC 7:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
					break;
			}
			break;
		default:
		...
		case 0x00:  /* FILD Bit16s */
			...
			break;
		case 0x01:
			LOG(LOG_FPU,LOG_WARN)("ESC 7 EA:Unhandled group %d subfunction %d",(int)decode.modrm.reg,(int)decode.modrm.rm);
			break;
