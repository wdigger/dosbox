	...
	CASE_0F_B(0x20)												/* MOV Rd.CRx */
		{
			...
			if (rm < 0xc0 ) {
				rm |= 0xc0;
				LOG(LOG_CPU,LOG_ERROR)("MOV XXX,CR%u with non-register",(unsigned int)which);
			}
			GetEArd;
			Bit32u crx_value;
	...
	CASE_0F_B(0x21)												/* MOV Rd,DRx */
		{
			...
			if (rm < 0xc0 ) {
				rm |= 0xc0;
				LOG(LOG_CPU,LOG_ERROR)("MOV XXX,DR%u with non-register",(unsigned int)which);
			}
			GetEArd;
			Bit32u drx_value;
	...
	CASE_0F_B(0x23)												/* MOV DRx,Rd */
		{
			...
			if (rm < 0xc0 ) {
				rm |= 0xc0;
				LOG(LOG_CPU,LOG_ERROR)("MOV DR%u,XXX with non-register",(unsigned int)which);
			}
			GetEArd;
	...
	CASE_0F_B(0x24)												/* MOV Rd,TRx */
		{
			...
			if (rm < 0xc0 ) {
				rm |= 0xc0;
				LOG(LOG_CPU,LOG_ERROR)("MOV XXX,TR%u with non-register",(unsigned int)which);
			}
			GetEArd;
			Bit32u trx_value;
	...
	CASE_0F_B(0x26)												/* MOV TRx,Rd */
		{
			...
			if (rm < 0xc0 ) {
				rm |= 0xc0;
				LOG(LOG_CPU,LOG_ERROR)("MOV TR%u,XXX with non-register",(unsigned int)which);
			}
			GetEArd;
