...
/* Using the instructions.h defines */
	...
	case O_GRP6d:
		...
		case 0x05:	/* VERW */
			...
			goto nextopcode;		/* Else value will saved */
		default:
			LOG(LOG_CPU,LOG_ERROR)("Group 6 Illegal subfunction %X",(unsigned int)inst.rm_index);
			goto illegalopcode;
		}
		break;
	case O_GRP7w:
	case O_GRP7d:
		...
		case 7:		/* INVLPG */
			...
			goto nextopcode;
		default:
			LOG(LOG_CPU,LOG_ERROR)("Group 7 Illegal subfunction %X",(unsigned int)inst.rm_index);
			goto illegalopcode;
		}
		break;
	case O_M_CRx_Rd:
	...
	case 0:
		break;
	default:
		LOG(LOG_CPU,LOG_ERROR)("OP:Unhandled code %d entry %X",inst.code.op,(unsigned int)inst.entry);
