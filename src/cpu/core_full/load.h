...
/* Others */		
		...
		case 0:
			break;
		default:
			LOG(LOG_CPU,LOG_ERROR)("MODRM:Unhandled load %d entry %x",inst.code.extra,(unsigned int)inst.entry);
			break;
		}
		break;
	case L_POPw:
...
/* Direct operations */
	...
	default:
		LOG(LOG_CPU,LOG_ERROR)("LOAD:Unhandled code %d opcode %X",inst.code.load,(unsigned int)inst.entry);
		goto illegalopcode;
}
