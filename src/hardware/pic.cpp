...
static void write_command(Bitu port,Bitu val,Bitu iolen) {
	...
	if (GCC_UNLIKELY(val&0x10)) {		// ICW1 issued
		...
		if (val&0x40) {		// special mask select
			...
			pic->check_for_irq();
			LOG(LOG_PIC,LOG_NORMAL)("port %X : special mask %s",(unsigned int)port,(pic->special)?"ON":"OFF");
		}
	} else {	// OCW2 issued
...
static void write_data(Bitu port,Bitu val,Bitu iolen) {
	...
	case 0:                        /* mask register */
		...
		break;
	case 1:                        /* icw2          */
		LOG(LOG_PIC,LOG_NORMAL)("%d:Base vector %X",port==0x21 ? 0 : 1,(unsigned int)val);
		...
		break;
	case 2:							/* icw 3 */
		LOG(LOG_PIC,LOG_NORMAL)("%d:ICW 3 %X",port==0x21 ? 0 : 1,(unsigned int)val);
		if(pic->icw_index++ >= pic->icw_words) pic->icw_index=0;
		break;
	case 3:							/* icw 4 */
		/*
		...
		*/
		...
		LOG(LOG_PIC,LOG_NORMAL)("%d:ICW 4 %X",port==0x21 ? 0 : 1,(unsigned int)val);

		if ((val&0x01)==0) E_Exit("PIC:ICW4: %x, 8085 mode not handled",(unsigned int)val);
		if ((val&0x10)!=0) LOG_MSG("PIC:ICW4: %x, special fully-nested mode not handled",(unsigned int)val);

		if(pic->icw_index++ >= pic->icw_words) pic->icw_index=0;
		break;
	default:
		LOG(LOG_PIC,LOG_NORMAL)("ICW HUH? %X",(unsigned int)val);
		break;
	}
}
