...
//TODO Base address
	...
	case 0x6b:	// BIOS scratchpad: LFB address
		...
		break;
	default:
		LOG(LOG_VGAMISC,LOG_NORMAL)("VGA:S3:CRTC:Write to illegal index %2X", (unsigned int)reg );
		break;
	}
}
...
void SVGA_S3_WriteSEQ(Bitu reg,Bitu val,Bitu iolen) {
	...
	case 0x15:
		...
		break;
	default:
		LOG(LOG_VGAMISC,LOG_NORMAL)("VGA:S3:SEQ:Write to illegal index %2X", (unsigned int)reg );
		break;
	}
}
...
Bitu SVGA_S3_ReadSEQ(Bitu reg,Bitu iolen) {
	...
	default:
		LOG(LOG_VGAMISC,LOG_NORMAL)("VGA:S3:SEQ:Read from illegal index %2X", (unsigned int)reg);
		return 0;
	}
}
