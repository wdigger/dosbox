...
//		LOG_DEBUG("Bit mask %2X",val);
	...
	default:
		...
		if (gfx(index) == 9 && !index9warned) {
			LOG(LOG_VGAMISC,LOG_NORMAL)("VGA:3CF:Write %2X to illegal index 9",(unsigned int)val);
			index9warned=true;
			break;
		}
		LOG(LOG_VGAMISC,LOG_NORMAL)("VGA:3CF:Write %2X to illegal index %2X",(unsigned int)val,gfx(index));
		break;
	}
}
...
static Bitu read_p3cf(Bitu port,Bitu iolen) {
	...
	default:
		...
		LOG(LOG_VGAMISC,LOG_NORMAL)("Reading from illegal index %2X in port %4X",static_cast<Bit32u>(gfx(index)),(unsigned int)port);
		break;
	}
